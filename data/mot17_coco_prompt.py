...
class MOT17PromptCOCO(MOTDataset):
    def __init__(self, config: dict, split: str, transform):
        super(MOT17PromptCOCO, self).__init__(config=config, split=split, transform=transform)
        self.spliter = "/"
        if os.name == "nt":
            self.spliter="\\"
        self.config = config
        ...
        self.mot17_seqs_dir = os.path.join(config["DATA_ROOT"], config["DATASET"].split("Prompt")[0], "images", split)
        self.mot17_gts_dir = os.path.join(config["DATA_ROOT"], config["DATASET"].split("Prompt")[0], "gts", split)
